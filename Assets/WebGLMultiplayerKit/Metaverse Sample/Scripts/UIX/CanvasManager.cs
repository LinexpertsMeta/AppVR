...
namespace MetaverseSample
{
    public class CanvasManager : MonoBehaviour
    {
        ...
        public GameObject avatarSelectorPanel;
        ...
        public TextMeshProUGUI userVoiceSound;
        ...
        public bool megafono;
        public bool compartirPantalla;

        public GameObject imageNotification;
        public List<ChatBox> chatBoxs;
        ...
        private void Update()
        {
            ...
            if (chatRoom.activeInHierarchy)
            {
                if (inputFieldMessage.isFocused)
                {
                    ...
                    {
                        NetworkManager.instance.EmitMessage();
                    }
                }
                else
                {
        ...
        public void OpenScreen(int _current)
        {
            switch (_current)
            {
                ...
                case 2:
                    ...
                    avatarSelectorPanel.SetActive(true);
                    break;
        ...
        public void ToggleMegafono()
        {
            SetMegafono();
            if (!megafono)
            {
                NetworkManager.instance.EmitMegafono();
            }
            else
            {

                NetworkManager.instance.EmitMegafonoOff();
            }
            megafono = !megafono;
        }
        ...
        public void TogglePantalla()
        {
            ...
            {
                Debug.Log("isSharring: " + s.isSharing);
                if (s.zoneToTransmit == MetaverseSample.NetworkManager.instance.currentZone && !s.isSharing)
                {
                    PlayerManager.instance.isSharing = true;
                    NetworkManager.instance.EmitUserSharingScreen(true);
                    NetworkManager.instance.EmitShareScreen();
                }
                else
                {
                    Debug.Log("No se puede transmitir porque alguien mas ya esta transmitiendo");
                }
            }
            //if (!compartirPantalla)
            //{
            //}
            //else
            //{
        ...
        public void SetUpUser(string _id, string _name, string _publicAddress)
        {
            ...
            newUser.transform.position = contentUsers.transform.position + (Vector3.right * 2) + (Vector3.down);
            ...
            newVoiceUser.GetComponent<VoiceUser>().txtName.text = _name;
            newVoiceUser.GetComponent<VoiceUser>().id = _id;
            newVoiceUser.transform.parent = contentVoiceUsers.transform;
            newVoiceUser.transform.position = contentVoiceUsers.transform.position + (Vector3.right * 2) + (Vector3.down);
        ...
        public void ClearUsersList()
        {
            ...
            foreach (GameObject user in voiceUsersList)
            {
        ...
        public void SpawnNetworkMessage(string _user_name, string _message)
        {
            ...
            newMessage.transform.position = contentMessages.transform.position + (Vector3.right * 2);
        ...
        public void SpawnMyMessage(string _message)
        {
            ...
            newMessage.GetComponent<Message>().txtMsg.text = _message;
            newMessage.transform.parent = contentMessages.transform;
            newMessage.transform.position = contentMessages.transform.position + (Vector3.right);
            newMessage.GetComponent<RectTransform>().localScale = new Vector3(1, 1, 1);
            messages.Add(newMessage);
        ...
        public void SpawnChatBox(string _id, string _host_id, string _guest_id, string _profileName)
        {
            ...
            foreach (ChatBox chat in chatBoxs)
            {
                if (((chat.host_id == NetworkManager.instance.local_player_id && chat.guest_id == _guest_id)
                    || (chat.host_id == _guest_id && chat.guest_id == NetworkManager.instance.local_player_id)))
                {
            ...
            chatBoxs.Add(newChatBox.GetComponent<ChatBox>());
        }
        ...
        public void CloseChatBox(ChatBox _chat_box)
        {
            ...
            Debug.Log("Cerrar chat box");
            _chat_box.gameObject.SetActive(false);
        }
        ...
        public void SetMic()
        {
            ...
            if (openMic)
            {
                micImg.sprite = micSprites[1];


            }
            else
            {
                micImg.sprite = micSprites[0];
        ...
        public void SetCam()
        {
            ...
            openCam = !openCam;     //allow or block mic

            if (openCam)
            {
                camImg.sprite = camSprites[1];


            }
            else
            {
                camImg.sprite = camSprites[0];

            }
        }

        public void SetMegafono()
        {

            openScreen = !openScreen;     //allow or block mic

            if (openScreen)
            {
                megafonoImg.sprite = megafonoSprites[1];


            }
            else
            {
                megafonoImg.sprite = megafonoSprites[0];
